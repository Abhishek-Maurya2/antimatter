...
class BlockerService {
  ...
  Future<void> _enforceWindowsBlocklist(List<String> blockedApps) async {
    for (String processName in blockedApps) {
      ...
      final exeName = processName.endsWith('.exe')
          ? processName
          : '$processName.exe';
      ...
      try {
        ...
        if (output.toLowerCase().contains(baseName.toLowerCase())) {
          ...
          if (killResult.exitCode == 0) {
            ...
            debugPrint(
              "Taskkill failed for $exeName with exit code ${killResult.exitCode}: ${killResult.stderr}",
            );
          }
        }
      } catch (e) {
  ...
  Future<void> _enforceAndroidBlocklist(List<String> blockedApps) async {
    ...
    try {
      ...
      if (latestForegroundEvent != null &&
          latestForegroundEvent.packageName != null) {
        final currentPackage = latestForegroundEvent.packageName!;

        bool isBlocked = blockedApps.any(
          (blocked) =>
        ...
        if (isBlocked) {
          debugPrint(
            "Blocked application detected in foreground: $currentPackage",
          );
          await NotificationService().showBlockerNotification(
            currentPackage.split('.').last,
          );
          ...
          try {
            ...
            // } else {
            //   debugPrint(
            //     "Failed to launch AntiMatter: startApp returned false.",
            //   );
            // }
          } catch (e) {
            ...
            try {
              ...
              if (await canLaunchUrl(url)) {
                ...
                debugPrint(
                  "Fallback URL scheme 'orches://' is not launchable.",
                );
              }
            } catch (urlError) {
